...
const IMAGE_API_URL =
  process.env.NEXT_PUBLIC_IMAGE_API_URL || "http://localhost:5000";
...
export const authApi = {
  ...
  updateProfile: (data: Partial<User>) => api.put("/api/auth/me", data),
};
...
export const diaryApi = {
  ...
  getDiary: (diaryId: number) => api.get<Diary>(`/api/diaries/${diaryId}`),
  ...
  deleteDiary: (diaryId: number) => api.delete(`/api/diaries/${diaryId}`),

  toggleLike: (diaryId: number) => api.post(`/api/diaries/${diaryId}/like`),
...
export const nodeApi = {
  ...
  deleteNode: (nodeId: number) => api.delete(`/api/nodes/${nodeId}`),
};
...
export const destinationsApi = {
  ...
  getRanking: (limit = 20) =>
    api.get<{ ranking: Array<Destination & { rank: number }> }>(
